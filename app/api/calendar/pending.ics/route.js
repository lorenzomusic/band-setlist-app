...
export async function GET() {
  try {
    ...
    // Filter for pending gigs
    const pendingGigs = gigs.filter(gig => gig.status === 'pending');
    
    // Generate iCal content
    const icalContent = generateICal(pendingGigs, true, members);
    
    return new Response(icalContent, {
      headers: {
        'Content-Type': 'text/calendar; charset=utf-8',
        'Content-Disposition': 'attachment; filename="pending-gigs.ics"'
      }
    });
  } catch (error) {
    console.error('Error generating pending gigs iCal:', error);
